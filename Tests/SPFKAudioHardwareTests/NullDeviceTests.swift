...
extension HardwareSuite.NullDeviceTests {
    @Test(arguments: [Scope.output, Scope.input])
    ...
    func setNominalSampleRate(sampleRate: Float64) async throws {
        ...
        // resetNullDeviceState() leaves the device at 44100, so that argument has no change to
        // observe until the device is moved off it first.
        if nullDevice.nominalSampleRate == sampleRate {
            try await nullDevice.sampleRateUpdater.updateAndWait(sampleRate: sampleRate == 44100 ? 48000 : 44100)
        }
        ...
        try await wait(sec: 2) {
            nullDevice.nominalSampleRate == sampleRate && nullDevice.actualSampleRate == sampleRate
        }
