...
extension AudioDevice {
    ...
    public func volumeInfo(channel: UInt32, scope: Scope) -> VolumeInfo? {
        ...
        if AudioBackend.current.hasProperty(id, address: &address) {
            var canSetVolumeBoolean = DarwinBoolean(false)
            var status = AudioBackend.current.isPropertySettable(id, address: &address, isSettable: &canSetVolumeBoolean)
        ...
        if AudioBackend.current.hasProperty(id, address: &address) {
            ...
            var status = AudioBackend.current.isPropertySettable(id, address: &address, isSettable: &canMuteBoolean)
        ...
        if AudioBackend.current.hasProperty(id, address: &address) {
            ...
            var status = AudioBackend.current.isPropertySettable(id, address: &address, isSettable: &canPlayThruBoolean)
